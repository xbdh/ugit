...
impl Migration {
    pub fn new(repo: Repo,tree_diff:IndexMap<PathBuf,(GHash,GHash)>) -> Self {
        ...
        println!("order_rm_dir: {:?}", order_rm_dir);
        println!("order_add_dir: {:?}", order_add_dir);
        Self {
            tree_diff,
            changes,
            order_rm_dir,
    ...
    pub fn update_workspace(&self){
        ...
        // update
        let modify_files = self.changes.get(&ChangeType::Modify).unwrap();
        ...
        // add
        let add_files = self.changes.get(&ChangeType::Add).unwrap();
    ...
    pub fn update_index(&self){
        ...
        for filename in add_files.iter(){
            let (old_hash,new_hash) = self.tree_diff.get(filename).unwrap();
            index.add(filename.clone(),new_hash.clone(),workspace.stat_file(filename));
        }
